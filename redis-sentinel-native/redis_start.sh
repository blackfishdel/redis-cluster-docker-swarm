...
redis-server ./redis2.conf --slaveof $REDIS_MASTER_IP $REDIS_MASTER_PORT
redis-server ./redis3.conf --slaveof $REDIS_MASTER_IP $REDIS_MASTER_PORT
...
for i in {1..3} ; do
  ...
  redis-server ./sentinel$i.conf --sentinel
done